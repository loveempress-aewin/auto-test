...
```
![suffixed](https://stackoverflow.com/questions/78366666/error-no-tests-found-make-sure-that-arguments-are-regular-expressions-matching)
...
>> Do you remember which package we used to download the environment initially? It was nvm, so give it a try
...
This is clearly an installation issue, right? So I don’t need to say much more (??).
The root cause is simply missing dependencies. It’s straightforward—just brute force it.
```bash
...
<optgroup label="Europe" data-select2-id="825"><option value="Europe/Andorra" data-select2-id="826">Europe/Andorra</option><option value="Europe/Tirane" data-select2-id="827">Europe/Tirane</option><option value="Europe/Vienna" data-select2-id="828">Europe/Vienna</option><option value="Asia/Taipei" data-select2-id="953">Asia/Taipei</option><option value="Asia/Samarkand" data-select2-id="954">Asia/Samarkand</option>
```
error write (it is my code --loveloveempress)
```js
...
Wow, finally I have time to write technical articles again…
Fri Jun  7 14:26:16 CST 2024

Today, I not only refactored many parts but also rewrote the code. Trust me, the programming queen is real! 👑
Recently, I’ve learned a lot of techniques, such as variable file splitting,
passing variables between files, return rules, and synonyms for ‘S.’
It’s frustrating—I came back after some time and realized I couldn’t understand my own code. 🤔

“If my ipmitool sel elist shows ‘SEL no entries,’ why doesn’t it display anything in wc? However, when I run it directly in Bash…
![elist_count](./pic/ipmitool_elist_wc.png)
```bash
...
printf "${sel_elist}">>./result/watchdog.txt
```
My assumption here is that there’s no information, and wc will automatically fill in zeros.
...
Found one moe filt --> `watchdog.txt；`
```bash
function_execute(){
    . ./auto_function_elist.sh 4;
    local_elist_count_old="${sel_elist_count}";
...
```
As soon as I investigated it,
I found that I saw the problem in the penultimate line

## detail --UTF8 ##
take a closer look at the logic behind code
`#x3B utf-8-unix` => `;`
